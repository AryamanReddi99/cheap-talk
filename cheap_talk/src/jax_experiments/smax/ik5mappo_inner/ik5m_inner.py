        ...
        def _train_loop(train_runner_state, unused):
            ...
            def _update_epoch(update_state, unused):
                def _update_minibatch(carry, minibatch_idx):
                    ...
                    # get k4 probs
                    actor_params_k4 = actor_train_state.params
                    _, pi_k4 = actor_network.apply(
                        actor_params_k4,
                        actor_hidden_state_init.squeeze(),
                        (
                            obs,
                    ...
                    log_prob_k4 = pi_k4.log_prob(action)
                    log_prob_k4_reshape = log_prob_k4.reshape(
                        -1, env.num_agents, config["NUM_ENVS"]
                    )
                    log_prob_k4_prod = jnp.sum(log_prob_k4_reshape, axis=1)
                    log_prob_k4_joint = jnp.tile(log_prob_k4_prod, (1, env.num_agents))

                    batch_k5 = (
                        log_prob_k4,
                        log_prob_k4_joint,
                    )
                    shuffled_batch_k5 = jax.tree.map(
                        lambda x: jnp.take(x, permutation, axis=1), batch_k5
                    )
                    shuffled_batch_reshaped_k5 = jax.tree.map(
                        lambda x: jnp.reshape(  # reshapes shuffled batch into separate minibatches by adding a dimension after actor dim
                            # e.g. advantages_stack (128,320) -> (128,2,160) if NUM_MINIBATCHES = 2
                            # traj_batch_stack.obs (128,320,127) -> (128,2,160,127)
                            x,
                    ...
                    minibatches_k5 = (
                        jax.tree.map(  # move minibatch dimension to the front
                            lambda x: jnp.moveaxis(x, 1, 0), shuffled_batch_reshaped_k5
                        )
                    )
                    minibatch_log_prob_k4 = minibatches_k5[0][minibatch_idx]
                    minibatch_log_prob_k4_joint = minibatches_k5[1][minibatch_idx]
                    ...
                    actor_grad_fn_k5 = jax.value_and_grad(
                        _actor_loss_fn_k5, has_aux=True
                    )
                    actor_loss_k5, actor_grads_k5 = actor_grad_fn_k5(
                        actor_train_state.params,
                        minibatch_actor_hidden_state_init,
                        minibatch_obs,
                        minibatch_done,
                        ...
                        minibatch_log_prob_k0,
                        minibatch_log_prob_k0_joint,
                        minibatch_log_prob_k4,
                        minibatch_log_prob_k4_joint,
                    )
                    actor_grad_norm_k5 = pytree_norm(actor_grads_k5)
                    actor_train_state = actor_train_state.apply_gradients(
                        grads=actor_grads_k5
                    )
                    ...
                    loss_info_k3 = {
                        ...
                        "actor_grad_norm_k4": actor_grad_norm_k4,
                        "actor_grad_norm_k5": actor_grad_norm_k5,
                    }
...
@hydra.main(version_base=None, config_path="./", config_name="config_ik5m_inner")
def main(config):
    try:
        config = OmegaConf.to_container(config)

        # WANDB
        job_type = f"iK5M_IN_{config['MAP_NAME']}"
        group = f"iK5M_IN_{config['MAP_NAME']}"
