        ...
        def _update_step(update_runner_state, unused):
            ...
            def _update_epoch(update_state, unused):
                def _update_minbatch(train_states, batch_info):
                    ...
                    actor_params_0 = actor_train_state.params
                    actor_opt_state_0 = actor_train_state.opt_state
                    ...
                    # --- extragradient correction: re-evaluate the same loss at the extrapolated point ---
                    actor_loss_eg, actor_grads_eg = actor_grad_fn(
                        actor_train_state.params, ac_init_hstate, traj_batch, advantages
                    )
                    actor_grad_norm_eg = pytree_norm(actor_grads_eg)
                    cosine_similarity = pytree_cosine_similarity(actor_grads, actor_grads_eg)

                    # --- step from the base point with the pre-probe optimizer state ---
                    actor_train_state = actor_train_state.replace(
                        params=actor_params_0, opt_state=actor_opt_state_0
                    )
                    actor_train_state = actor_train_state.apply_gradients(grads=actor_grads_eg)
                    ...
                    loss_info = {
                        ...
                        "actor_loss_eg": actor_loss_eg[0],
                        "entropy_eg": actor_loss_eg[1][1],
                        "ratio_eg": actor_loss_eg[1][2],
                        "approx_kl_eg": actor_loss_eg[1][3],
                        "clip_frac_eg": actor_loss_eg[1][4],
                        "actor_grad_norm_eg": actor_grad_norm_eg,
...
def main(config):
    try:
        ...
        # WANDB
        group = f"EGMAPPO_{config['MAP_NAME']}"
        job_type = f"EGMAPPO_{config['MAP_NAME']}"
