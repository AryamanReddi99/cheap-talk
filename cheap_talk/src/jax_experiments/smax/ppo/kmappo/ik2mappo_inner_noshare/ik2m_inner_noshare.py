...
from cheap_talk.src.utils.jax_utils import pytree_norm, pytree_cosine_similarity
...
class ActorRNN(nn.Module):
    ...
    def __call__(self, hidden, x):
        ...
        embedding = nn.relu(embedding)  # embedding has 3d shape (1, num_actors, FC_DIM_SIZE)
    ...
    def train(rng, exp_id):
        ...
        def train_setup(rng):
            ...
            actor_network_params = jax.vmap(
                lambda rng: actor_network.init(rng, actor_hidden_state_init, ac_init_x)
            )(actor_init_rngs)
            cr_init_x = (
                jnp.zeros(
                    (
        ...
        def _train_loop(train_runner_state, unused):
            ...
            def _env_step(runner_state, unused):
                ...
                actor_hidden_state = actor_hidden_state.reshape(
                    (env.num_agents, config["NUM_ENVS"], actor_hidden_state.shape[-1])
                )
                ...
                actor_hidden_state_new, pi = jax.vmap(_actor_apply)(
                ...
                actor_hidden_state_new = actor_hidden_state_new.reshape((config["NUM_ACTORS"], -1))
                ...
                info_batched = jax.tree.map(lambda x: x.reshape((config["NUM_ACTORS"])), info)
                done_batched = batchify(done, env.agents, config["NUM_ACTORS"]).squeeze()
            ...
            def _update_epoch(update_state, unused):
                def _update_minibatch(carry, minibatch_idx):
                    ...
                    def _actor_loss_fn_k1(
                        ...
                        # RERUN NETWORK
                        num_envs = done.shape[1] // env.num_agents
                        obs = obs.reshape((config["NUM_STEPS"], env.num_agents, num_envs, -1))
                        done = done.reshape((config["NUM_STEPS"], env.num_agents, num_envs))
                        avail_actions = avail_actions.reshape(
                            (config["NUM_STEPS"], env.num_agents, num_envs, -1)
                        )
                        action = action.reshape((config["NUM_STEPS"], env.num_agents, num_envs))
                        log_prob_k0 = log_prob_k0.reshape(
                        ...
                        log_prob_k0 = jnp.swapaxes(log_prob_k0, 0, 1)
                        gae = jnp.swapaxes(gae, 0, 1)

                        actor_hidden_state_init = actor_hidden_state_init.reshape(
                    ...
                    actor_grad_fn_k1 = jax.value_and_grad(_actor_loss_fn_k1, has_aux=True)
                    ...
                    actor_train_state = actor_train_state.apply_gradients(grads=actor_grads_k1)
                    critic_train_state = critic_train_state.apply_gradients(grads=critic_grads)

                    # get k1 probs
                    actor_params_k1 = actor_train_state.params
                    num_envs = done.shape[1] // env.num_agents
                    obs_agent = obs.reshape((config["NUM_STEPS"], env.num_agents, num_envs, -1))
                    done_agent = done.reshape((config["NUM_STEPS"], env.num_agents, num_envs))
                    avail_actions_agent = avail_actions.reshape(
                        (config["NUM_STEPS"], env.num_agents, num_envs, -1)
                    )
                    action_agent = action.reshape((config["NUM_STEPS"], env.num_agents, num_envs))

                    obs_agent = jnp.swapaxes(obs_agent, 0, 1)
                    done_agent = jnp.swapaxes(done_agent, 0, 1)
                    avail_actions_agent = jnp.swapaxes(avail_actions_agent, 0, 1)
                    action_agent = jnp.swapaxes(action_agent, 0, 1)

                    actor_hidden_state_init_agent = actor_hidden_state_init.reshape(
                        (env.num_agents, num_envs, -1)
                    )
                    ...
                    pi_k1 = jax.vmap(_actor_apply)(
                        actor_params_k1,
                        actor_hidden_state_init_agent,
                        obs_agent,
                        done_agent,
                        avail_actions_agent,
                    )
                    log_prob_k1 = pi_k1.log_prob(action_agent)
                    log_prob_k1 = jnp.swapaxes(log_prob_k1, 0, 1).reshape((config["NUM_STEPS"], -1))
                    log_prob_new_k1 = jnp.exp(log_prob_k1 - log_prob_k0)
                    log_prob_k1_reshape = log_prob_k1.reshape(-1, env.num_agents, num_envs)
                    ...
                    minibatches_k2 = jax.tree.map(  # move minibatch dimension to the front
                    ...
                    def _actor_loss_fn_k2(
                        ...
                        log_prob_k0_joint = log_prob_k0_joint.reshape(
                            (config["NUM_STEPS"], env.num_agents, num_envs)
                        )
                        log_prob_k1 = log_prob_k1.reshape(
                            (config["NUM_STEPS"], env.num_agents, num_envs)
                        )
                        log_prob_k1_joint = log_prob_k1_joint.reshape(
                            (config["NUM_STEPS"], env.num_agents, num_envs)
                        )
                        gae = gae.reshape((config["NUM_STEPS"], env.num_agents, num_envs))
                        ...
                        action = jnp.swapaxes(action, 0, 1)
                        log_prob_k0 = jnp.swapaxes(log_prob_k0, 0, 1)
                        log_prob_k0_joint = jnp.swapaxes(log_prob_k0_joint, 0, 1)
                        log_prob_k1 = jnp.swapaxes(log_prob_k1, 0, 1)
                        log_prob_k1_joint = jnp.swapaxes(log_prob_k1_joint, 0, 1)
                        gae = jnp.swapaxes(gae, 0, 1)
                        ...
                        # CALCULATE ACTOR LOSS
                        logratio_is = log_prob + log_prob_k1_joint - log_prob_k0_joint - log_prob_k1
                        ...
                        ratio_is_k = jnp.exp(
                            log_prob_k1_joint - log_prob_k1 + log_prob_k0 - log_prob_k0_joint
                        )
                    ...
                    actor_grad_fn_k2 = jax.value_and_grad(_actor_loss_fn_k2, has_aux=True)
                    ...
                    actor_train_state = actor_train_state.apply_gradients(grads=actor_grads_k2)
                    actor_params_k2 = actor_train_state.params
                    pi_k2 = jax.vmap(_actor_apply)(
                        actor_params_k2,
                        actor_hidden_state_init_agent,
                        obs_agent,
                        done_agent,
                    ...
                    log_prob_k2 = pi_k2.log_prob(action_agent)
                    log_prob_k2 = jnp.swapaxes(log_prob_k2, 0, 1).reshape((config["NUM_STEPS"], -1))
                    ...
                    # cosine_similarity = pytree_cosine_similarity(actor_grads_k1, actor_grads_k2)
                    cosine_similarity = 0
                    loss_info = {
            ...
            def callback(exp_id, metric):
                log_dict = {
                    ...
                    "env_step": metric["update_step"] * config["NUM_ENVS"] * config["NUM_STEPS"],
                    **metric["loss"],
                    **metric["loss_k"],
                }
...
@hydra.main(version_base=None, config_path="./", config_name="config_ik2m_inner_noshare")
def main(config):
    try:
        config = OmegaConf.to_container(config)

        # WANDB
        job_type = f"iK2M_IN_NOSHARE_{config['MAP_NAME']}"
        group = f"iK2M_IN_NOSHARE_{config['MAP_NAME']}"
