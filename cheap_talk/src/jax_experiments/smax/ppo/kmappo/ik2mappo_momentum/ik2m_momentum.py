...
def pytree_dot(pytree1, pytree2):
    dots = jax.tree_util.tree_map(lambda x, y: jnp.sum(x * y), pytree1, pytree2)
...
def make_train(config):
    ...
    def train(rng, exp_id):
        ...
        def _train_loop(train_runner_state, unused):
            ...
            prev_update_1 = train_runner_state.prev_update_1
            prev_update_2 = train_runner_state.prev_update_2
            if config["RESET_MOMENTUM_EACH_UPDATE"]:
                prev_update_1 = jax.tree.map(jnp.zeros_like, prev_update_1)
                prev_update_2 = jax.tree.map(jnp.zeros_like, prev_update_2)
            ...
            def _update_epoch(update_state, unused):
                def _update_minibatch(carry, minibatch_idx):
                    ...
                    # --- momentum gate: score the previous increment by its agreement
                    # --- with the one before it, and use it as the level-1 proxy
                    cos_uu = pytree_cosine_similarity(prev_update_1, prev_update_2)
                    gate_c = jnp.clip(cos_uu, 0.0, 1.0)
                    proxy_update = jax.tree.map(lambda u: gate_c * u, prev_update_1)
                    u1_norm = pytree_norm(prev_update_1)
                    u2_norm = pytree_norm(prev_update_2)
                    ...
                    # --- AUDIT: score the proxy against the real level-1 displacement.
                    # --- Purely observational: params and opt_state are restored, so the
                    # --- applied update is byte-identical whether AUDIT is on or off.
                    if config["AUDIT"]:
                        actor_opt_state_k0 = actor_train_state.opt_state
                        actor_grad_fn_k1_audit = jax.value_and_grad(
                            _actor_loss_fn_k1, has_aux=True
                        )
                        _, actor_grads_k1_audit = actor_grad_fn_k1_audit(
                            actor_params_k0,
                        ...
                        actor_train_state_audit = actor_train_state.apply_gradients(
                            grads=actor_grads_k1_audit
                        )
                        d_star = jax.tree.map(
                            lambda a, b: a - b,
                            actor_train_state_audit.params,
                            actor_params_k0,
                        )
                        ...
                        cos_phi = pytree_cosine_similarity(prev_update_1, d_star)
                        d_star_norm = pytree_norm(d_star)
                        c_star = pytree_dot(prev_update_1, d_star) / jnp.maximum(
                            u1_norm ** 2, 1e-12
                        )
                        scale_ratio = d_star_norm / jnp.maximum(u1_norm, 1e-12)
                        sin_phi = jnp.sqrt(jnp.clip(1.0 - cos_phi ** 2, 0.0, 1.0))
                        anticip_err = pytree_norm(
                            jax.tree.map(lambda a, b: a - b, proxy_update, d_star)
                        ) / jnp.maximum(d_star_norm, 1e-12)

                    # get k1 probs at the proxy point instead of the real level-1 params
                    actor_params_k1 = jax.tree.map(
                        lambda p, u: p + u, actor_params_k0, proxy_update
                    )
                    ...
                    new_update = jax.tree.map(
                        lambda a, b: a - b, actor_train_state.params, actor_params_k0
                    )
                    ...
                    total_loss = actor_loss_k2[0] + critic_loss[0]
                    loss_info = {
                        "total_loss": total_loss,
                        "actor_loss": actor_loss_k2[0],
                        "value_loss": critic_loss[0],
                        "entropy": actor_loss_k2[1][1],
                        "ratio": actor_loss_k2[1][2],
                        "approx_kl": actor_loss_k2[1][3],
                        "clip_frac": actor_loss_k2[1][4],
                        "actor_grad_norm": actor_grad_norm_k2,
                    ...
                    if config["AUDIT"]:
                        loss_info["cos_phi"] = cos_phi
                        loss_info["d_star_norm"] = d_star_norm
                        loss_info["c_star"] = c_star
                        loss_info["scale_ratio"] = scale_ratio
                        loss_info["sin_phi"] = sin_phi
                        loss_info["anticip_err"] = anticip_err
                    loss_info_k = {
                ...
                prev_update_1 = update_state.prev_update_1
                prev_update_2 = update_state.prev_update_2
                ...
                update_state = update_state.replace(
                    ...
                    rng=rng,
                    prev_update_1=train_states[14],
                    prev_update_2=train_states[15],
                )
            ...
            initial_update_state = UpdateRunnerState(
                ...
                rng=rng,
                prev_update_1=prev_update_1,
                prev_update_2=prev_update_2,
            )
            ...
            prev_update_1 = final_update_state.prev_update_1
            prev_update_2 = final_update_state.prev_update_2
        ...
        initial_runner_state = TrainRunnerState(
            ...
            update_step=0,
            rng=_train_rng,
            prev_update_1=jax.tree.map(jnp.zeros_like, actor_train_state.params),
            prev_update_2=jax.tree.map(jnp.zeros_like, actor_train_state.params),
        )
...
@hydra.main(version_base=None, config_path="./", config_name="config_ik2m_momentum")
def main(config):
    try:
        config = OmegaConf.to_container(config)

        # WANDB
        job_type = f"iK2M_MOM_{config['MAP_NAME']}"
        group = f"iK2M_MOM_{config['MAP_NAME']}"
        if config["AUDIT"]:
            job_type = f"iK2M_MOMAUDIT_{config['MAP_NAME']}"
            group = f"iK2M_MOMAUDIT_{config['MAP_NAME']}"
