            ...
            def _learn_phase(carry, _):
                ...
                def _critic_loss_fn(critic_params):
                    ...
                    _, target_q_next = jax.vmap(
                        ...
                        critic_train_state.target_network_params,
                        init_critic_hs,
                        _obs,
                        _dones,
                    ...
                    target_q = _rewards[:, :-1] + config["GAMMA"] * target_q_next[
                        :, 1:
                    ] * (1 - _dones[:, :-1])

                    # MSE loss
                    loss = jnp.mean(
                        (q_vals[:, :-1] - jax.lax.stop_gradient(target_q)) ** 2
                    )
                    return loss
                ...
                def _actor_loss_fn(actor_params):
                    ...
                    # Maximize Q-value (negative for minimization)
                    loss = -jnp.mean(q_vals)
                    return loss
